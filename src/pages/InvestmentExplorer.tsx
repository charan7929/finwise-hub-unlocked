...
import { Wallet, ChartLine, LineChart, BookOpen, PieChart, BarChart, TrendingUp, ShieldCheck, AlertTriangle, Info } from "lucide-react";
import { ChartContainer } from "@/components/ui/chart";
import { ResponsiveContainer, AreaChart, Area, LineChart as RLineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, BarChart as RBarChart, Bar } from "recharts";

// Mock data for demonstration purposes
const INVESTMENT_TYPES = [
  { id: "mutual_funds", name: "Mutual Funds", icon: PieChart },
  { id: "sips", name: "Systematic Investment Plans", icon: ChartLine },
  { id: "etfs", name: "Exchange Traded Funds", icon: BarChart },
    ...
    <div className="container mx-auto py-6 max-w-7xl">
      ...
      >
        ...
        <TabsContent value="etfs">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ETFS.map((etf) => (
              <Card key={etf.id} className={`${selectedFunds.includes(etf.id) ? 'ring-2 ring-finwise-blue' : ''}`}>
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle>{etf.name}</CardTitle>
                      <CardDescription className="mt-1">{etf.description}</CardDescription>
                    </div>
                    {showCompare && (
                      <Button
                        size="sm"
                        variant={selectedFunds.includes(etf.id) ? "default" : "outline"}
                        onClick={() => toggleFundSelection(etf.id)}
                        className={selectedFunds.includes(etf.id) ? "bg-finwise-blue hover:bg-finwise-blue/90" : ""}
                      >
                        {selectedFunds.includes(etf.id) ? "Selected" : "Compare"}
                      </Button>
                    )}
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-muted-foreground">Risk Level</p>
                        <p className={`mt-1 px-2 py-0.5 rounded text-xs inline-block ${getRiskColor(etf.riskLevel)}`}>
                          {etf.riskLevel}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Expected Returns</p>
                        <p className="font-medium mt-1">{etf.expectedReturns}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Price</p>
                        <p className="font-medium mt-1">${etf.price}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Expense Ratio</p>
                        <p className="font-medium mt-1">{etf.expense}%</p>
                      </div>
                    </div>
                    ...
                    <div className="h-[150px] pt-4">
                      ...
                      >
                        <ResponsiveContainer width="100%" height="100%">
                          <RBarChart data={etf.performance} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.2} />
                            <XAxis dataKey="year" tick={{ fontSize: 10 }} />
                            <YAxis
                              tickFormatter={(value) => `${value}%`}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {etf.categories.map((category, index) => (
                          <span key={index} className="px-2 py-1 bg-muted rounded-full text-xs">
                            {category}
                          </span>
                        ))}
                    ...
                    <div className="pt-2">
                      ...
                      <div className="mt-2 p-2 bg-muted rounded-md text-center">
                        <p className="text-sm text-muted-foreground">Potential Value</p>
                        <p className="font-bold text-lg">
                          ${(parseInt(investmentAmount || "0") * (1 + 
                            etf.performance.reduce((sum, item) => sum + item.return, 0) / 
                            etf.performance.length / 100 * 5)).toFixed(2)}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <TabsContent value="bonds">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {BONDS.map((bond) => (
              <Card key={bond.id} className={`${selectedFunds.includes(bond.id) ? 'ring-2 ring-finwise-blue' : ''}`}>
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle>{bond.name}</CardTitle>
                      <CardDescription className="mt-1">{bond.description}</CardDescription>
                    </div>
                    {showCompare && (
                      <Button
                        size="sm"
                        variant={selectedFunds.includes(bond.id) ? "default" : "outline"}
                        onClick={() => toggleFundSelection(bond.id)}
                        className={selectedFunds.includes(bond.id) ? "bg-finwise-blue hover:bg-finwise-blue/90" : ""}
                      >
                        {selectedFunds.includes(bond.id) ? "Selected" : "Compare"}
                      </Button>
                    )}
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-muted-foreground">Risk Level</p>
                        <p className={`mt-1 px-2 py-0.5 rounded text-xs inline-block ${getRiskColor(bond.riskLevel)}`}>
                          {bond.riskLevel}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Expected Returns</p>
                        <p className="font-medium mt-1">{bond.expectedReturns}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Yield</p>
                        <p className="font-medium mt-1">{bond.yield}%</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Duration</p>
                        <p className="font-medium mt-1">{bond.duration} years</p>
                      </div>
                    </div>

                    {/* Performance Chart */}
                    <div className="h-[150px] pt-4">
                      <p className="text-sm font-medium mb-2">Historical Yield</p>
                      <ChartContainer
                        config={{
                          return: {
                            label: "Annual Return",
                      ...
                      >
                        <ResponsiveContainer width="100%" height="100%">
                          <RLineChart data={bond.performance} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                            <XAxis dataKey="year" tick={{ fontSize: 10 }} />
                            <YAxis
                              tickFormatter={(value) => `${value}%`}
                    ...
                    <div>
                      <p className="text-sm font-medium mb-2">Categories</p>
                      <div className="flex flex-wrap gap-2">
                        {bond.categories.map((category, index) => (
                          <span key={index} className="px-2 py-1 bg-muted rounded-full text-xs">
                            {category}
                          </span>
                        ))}
                    ...
                    <div className="bg-yellow-50 p-3 rounded-md">
                      <div className="flex items-start">
                        ...
                        <div>
                          <p className="text-sm font-medium text-yellow-800">Investment Note</p>
                          <p className="text-xs text-yellow-700">
                            Bonds typically offer lower returns than equities but provide more stability and income.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Simulate Investment */}
                    <div className="pt-2">
                      <p className="text-sm font-medium mb-2">Calculate Returns</p>
                      <div className="flex space-x-2">
                        <div className="relative flex-1">
                          <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-muted-foreground">$</span>
                          <Input
                        ...
                        <Button className="shrink-0">Calculate</Button>
                      </div>
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        <div className="p-2 bg-muted rounded-md text-center">
                          <p className="text-xs text-muted-foreground">Annual Income</p>
                          <p className="font-bold">
                            ${(parseInt(investmentAmount || "0") * bond.yield / 100).toFixed(2)}
                          </p>
                        </div>
                        <div className="p-2 bg-muted rounded-md text-center">
                          <p className="text-xs text-muted-foreground">Maturity Value</p>
                          <p className="font-bold">
                            ${(parseInt(investmentAmount || "0") * Math.pow(1 + bond.yield / 100, bond.duration)).toFixed(2)}
                          </p>
                        </div>
                      </div>
                    </div>
